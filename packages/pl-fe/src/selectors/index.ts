...
const selectAccounts = (state: RootState, accountIds: Array<string>) =>
  accountIds.map(
    (accountId) =>
      state.entities[Entities.ACCOUNTS]?.store[accountId] as
        | Account
        | undefined,
  );
...
const getAccountBase = (state: RootState, accountId: string) =>
  state.entities[Entities.ACCOUNTS]?.store[accountId] as Account | undefined;
const getAccountRelationship = (state: RootState, accountId: string) =>
  state.relationships.get(accountId);
const getAccountMeta = (state: RootState, accountId: string) =>
  state.accounts_meta[accountId];

const makeGetAccount = () =>
  createSelector(
    [getAccountBase, getAccountRelationship, getAccountMeta],
...
const checkFiltered = (index: string, filters: ImmutableList<Filter>) =>
  filters.reduce(
    (result: Array<string>, filter) =>
      result.concat(
        filter.keywords.reduce((result: Array<string>, keyword) => {
          let expr = escapeRegExp(keyword.keyword);
  ...
  createSelector(
    [
      ...
      (state: RootState, { id }: APIStatus) =>
        state.statuses.get(state.statuses.get(id)?.reblog_id || '', null),
      (state: RootState, { id }: APIStatus) =>
        state.statuses.get(state.statuses.get(id)?.quote_id || '', null),
      (state: RootState, { id }: APIStatus) => {
        const group = state.statuses.get(id)?.group_id;
        if (group)
          return state.entities[Entities.GROUPS]?.store[group] as Group;
        return undefined;
      },
    ...
    (
      ...
      const filtered = features.filtersV2
        ...
        : (features.filters &&
            account.id !== me &&
            checkFiltered(
              statusReblog?.search_index || statusBase.search_index || '',
              filters,
            )) ||
          [];
...
type SelectedStatus = Exclude<
  ReturnType<ReturnType<typeof makeGetStatus>>,
  null
>;
...
const makeGetNotification = () =>
  createSelector(
    [
      ...
      // @ts-ignore
      (state: RootState, notification: MinifiedNotification) =>
        selectAccount(state, normalizeId(notification.account_id)),
      // @ts-ignore
      (state: RootState, notification: MinifiedNotification) =>
        selectAccount(state, normalizeId(notification.target_id)),
      // @ts-ignore
      (state: RootState, notification: MinifiedNotification) =>
        state.statuses.get(normalizeId(notification.status_id)),
      (state: RootState, notification: MinifiedNotification) =>
        notification.account_ids
          ? selectAccounts(state, notification.account_ids?.map(normalizeId))
          : null,
    ],
...
const getAccountGallery = createSelector(
  [
    (state: RootState, id: string) =>
      state.timelines.get(`account:${id}:with_replies:media`)?.items ||
      ImmutableOrderedSet<string>(),
    (state: RootState) => state.statuses,
  ],
  (statusIds, statuses) =>
    statusIds.reduce(
      (medias: ImmutableList<AccountGalleryAttachment>, statusId: string) => {
...
const getGroupGallery = createSelector(
  [
    (state: RootState, id: string) =>
      state.timelines.get(`group:${id}:media`)?.items ||
      ImmutableOrderedSet<string>(),
    (state: RootState) => state.statuses,
  ],
  (statusIds, statuses) =>
...
const makeGetReport = () => {
  ...
  return createSelector(
    [
      ...
      (state: RootState, id: string) =>
        selectAccount(state, state.admin.reports.get(id)?.account_id || ''),
      (state: RootState, id: string) =>
        selectAccount(
          state,
          state.admin.reports.get(id)?.target_account_id || '',
        ),
      (state: RootState, id: string) =>
        state.admin.reports
          .get(id)!
          .status_ids.map((id) => getStatus(state, { id }))
...
const makeGetOtherAccounts = () =>
  createSelector(
    [
      (state: RootState) =>
        state.entities[Entities.ACCOUNTS]?.store as EntityStore<Account>,
      getAuthUserIds,
      (state: RootState) => state.me,
    ],
...
const getRemoteInstanceFavicon = (state: RootState, host: string) => {
  const accounts = state.entities[Entities.ACCOUNTS]
    ?.store as EntityStore<Account>;
...
const getRemoteInstanceFederation = (
  state: RootState,
  host: string,
): HostFederation => {
