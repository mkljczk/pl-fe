...
describe('updatePageItem()', () => {
  ...
  describe('without cached data', () => {
    it('safely returns undefined', () => {
      ...
      const nextQueryData =
        queryClient.getQueryData<InfiniteData<PaginatedResult<Item>>>(queryKey);
  ...
  describe('with cached data', () => {
    ...
    it('updates the correct item in the cached data', () => {
      const initialQueryData =
        queryClient.getQueryData<InfiniteData<PaginatedResult<Item>>>(queryKey);
      expect(initialQueryData?.pages[0].result.map((r) => r.id)).toEqual([
        1, 2, 3,
      ]);
      expect(
        initialQueryData?.pages[0].result.find((r) => r.id === 2)?.text,
      ).toEqual('item 2');
      updatePageItem<Item>(
        queryKey,
        { id: 2, text: 'new text' },
      ...
      const nextQueryData =
        queryClient.getQueryData<InfiniteData<PaginatedResult<Item>>>(queryKey);
      expect(nextQueryData?.pages[0].result.map((r) => r.id)).toEqual([
        1, 2, 3,
      ]);
      expect(
        nextQueryData?.pages[0].result.find((r) => r.id === 2)?.text,
      ).toEqual('new text');
    });
  });
});
