...
const EmbeddedStatus = React.lazy(
  () => import('pl-fe/features/embedded-status'),
);
...
const PlFeMount = () => {
  ...
  const shouldUpdateScroll = (prevRouterProps, { location }) =>
    !(
      location.state?.plFeModalKey &&
      location.state?.plFeModalKey !==
        prevRouterProps?.location?.state?.plFeModalKey
    ) &&
    !(
      location.state?.plFeDropdownKey &&
      location.state?.plFeDropdownKey !==
        prevRouterProps?.location?.state?.plFeDropdownKey
    );
            ...
            <Switch>
              ...
              <Route>
                <Suspense fallback={<LoadingScreen />}>
                  {showOnboarding ? <OnboardingWizard /> : <UI />}
                </Suspense>
