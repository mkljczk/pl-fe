...
import { useLocale, usePlFeConfig, useSettings, useTheme } from 'pl-fe/hooks';
...
const PlFeHead: React.FC<IPlFeHead> = ({ children }) => {
  ...
  const { demo, reduceMotion, underlineLinks, demetricator, systemFont } =
    useSettings();
  ...
  const themeCss = generateThemeCss(
    demo ? normalizePlFeConfig({ brandColor: '#d80482' }) : plFeConfig,
  );
  const dsn = plFeConfig.sentryDsn;

  const bodyClass = clsx(
    'h-full bg-white text-base antialiased black:bg-black dark:bg-gray-800',
    {
