...
const ReadMoreButton: React.FC<IReadMoreButton> = ({
  onClick,
  quote,
  poll,
}) => (
  <div className='relative -mt-4'>
    <div
      className={clsx(
        'absolute -top-16 h-16 w-full bg-gradient-to-b from-transparent',
        {
          'to-white black:to-black dark:to-primary-900': !poll,
          'to-gray-100 dark:to-primary-800': poll,
          'group-hover:to-gray-100 black:group-hover:to-gray-800 dark:group-hover:to-gray-800':
            quote,
        },
      )}
    />
...
const StatusContent: React.FC<IStatusContent> = React.memo(
  ({
    ...
    const parsedHtml = useMemo(
      (): string =>
        translatable && status.translation
    ...
    useEffect(() => {
      setLineClamp(
        !spoilerNode.current || spoilerNode.current.clientHeight >= 96,
      );
    }, [spoilerNode.current]);
    ...
    const spoilerText =
      status.spoilerMapHtml && status.currentLanguage
        ? status.spoilerMapHtml[status.currentLanguage] || status.spoilerHtml
        : status.spoilerHtml;

    const direction = getTextDirection(status.search_index);
    const className = clsx(
      'relative overflow-hidden text-ellipsis break-words text-gray-900 focus:outline-none dark:text-gray-100',
      {
        ...
        <Text key='spoiler' size='2xl' weight='medium'>
          ...
          {content && expandable && (
            ...
            >
              {expanded ? (
                <FormattedMessage
                  id='status.spoiler.collapse'
                  defaultMessage='Collapse'
                />
              ) : (
                <FormattedMessage
                  id='status.spoiler.expand'
    ...
    if (onClick) {
      ...
      if (collapsed) {
        output.push(
          <ReadMoreButton onClick={() => {}} key='read-more' quote={quote} />,
        );
      }
