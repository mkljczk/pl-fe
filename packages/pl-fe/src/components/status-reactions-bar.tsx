...
const messages = defineMessages({
  emojiCount: {
    ...
    defaultMessage:
      '{count} {count, plural, one {person} other {people}} reacted with {emoji}',
  },
...
const StatusReaction: React.FC<IStatusReaction> = ({
  ...
  return (
    <button
      className={clsx(
        'group flex cursor-pointer items-center gap-2 overflow-hidden rounded-md border border-gray-400 px-1.5 py-1 transition-colors',
        {
          'bg-primary-100 dark:border-primary-400 dark:bg-primary-400 black:border-primary-600 black:bg-primary-600':
            reaction.me,
          'bg-transparent dark:border-primary-700 dark:bg-primary-700 black:border-primary-800 black:bg-primary-800':
            !reaction.me,
          'cursor-pointer': !unauthenticated,
          'hover:bg-primary-200 hover:dark:border-primary-300 hover:dark:bg-primary-300 hover:black:bg-primary-500':
            reaction.me && !unauthenticated,
          'hover:bg-primary-100 hover:dark:border-primary-600 hover:dark:bg-primary-600 hover:black:bg-primary-700':
            !reaction.me && !unauthenticated,
        },
      )}
      key={reaction.name}
...
const StatusReactionsBar: React.FC<IStatusReactionsBar> = ({
  status,
  collapsed,
}) => {
  const dispatch = useAppDispatch();
  ...
  if ((demetricator || status.emoji_reactions.length === 0) && collapsed)
    return null;
