...
const isMediaVisible = (
  ...
  else if (displayMedia === 'hide_all' && status.media_attachments.length)
    visible = false;
...
const showOverlay = (
  status: Pick<
    Status,
    'hidden' | 'media_attachments' | 'sensitive' | 'spoiler_text'
  >,
  displayMedia: 'default' | 'show_all' | 'hide_all',
) => {
  ...
  const showHideButton =
    status.sensitive ||
    (status.media_attachments.length && displayMedia === 'hide_all');
...
const SensitiveContentOverlay = React.forwardRef<
  HTMLDivElement,
