...
const TranslateButton: React.FC<ITranslateButton> = ({ status }) => {
  ...
  const knownLanguages = autoTranslate
    ? [...settings.knownLanguages, intl.locale]
    : [intl.locale];
  ...
  const renderTranslate =
    ...
    !status.contentMapHtml?.[intl.locale];
  ...
  const button = (
    <button className='w-fit' onClick={handleTranslate}>
      <HStack
        alignItems='center'
        space={1}
        className='text-primary-600 hover:underline dark:text-gray-600'
      >
        <Icon
        ...
        {status.translating && (
          <Icon
            src={require('@tabler/icons/outline/loader-2.svg')}
