...
import { type AdminReport, normalizeAdminReport } from 'pl-fe/normalizers';

import type {
  AdminAccount,
  AdminGetAccountsParams,
  AdminReport as BaseAdminReport,
} from 'pl-api';
...
const approveUser = (state: State, user: AdminAccount): State =>
  state.withMutations((state) => {
    ...
    state.update('awaitingApproval', (orderedSet) =>
      orderedSet.delete(user.id),
    );
    state.setIn(['users', user.id], normalizedUser);
  });

const minifyReport = (report: AdminReport) =>
  omit(report, [
    'account',
    'target_account',
...
const admin = (state: State = ReducerRecord(), action: AnyAction): State => {
  switch (action.type) {
    ...
    case ADMIN_USER_APPROVE_REQUEST:
      return state.update('awaitingApproval', (set) =>
        set.subtract(action.accountId),
      );
