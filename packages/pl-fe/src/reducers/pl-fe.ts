...
const updateFromAdmin = (
  state: ImmutableMap<string, any>,
  configs: ImmutableList<ImmutableMap<string, any>>,
) => {
  try {
    return ConfigDB.find(configs, ':pleroma', ':frontend_configurations')!
      .get('value')
      .find(
        (value: ImmutableMap<string, any>) =>
          value.getIn(['tuple', 0]) === ':pl_fe',
      )
      .getIn(['tuple', 1]);
  } catch {
    return state;
...
const persistPlFeConfig = (
  plFeConfig: ImmutableMap<string, any>,
  host: string,
) => {
  if (host) {
    KVStore.setItem(`plfe_config:${host}`, plFeConfig.toJS()).catch(
      console.error,
    );
  }
};

const importPlFeConfig = (
  state: ImmutableMap<string, any>,
  plFeConfig: ImmutableMap<string, any>,
  host: string,
) => {
...
const plfe = (state = initialState, action: Record<string, any>) => {
  switch (action.type) {
    ...
    case PLFE_CONFIG_REQUEST_SUCCESS:
      return importPlFeConfig(
        state,
        fromJS(action.plFeConfig) as ImmutableMap<string, any>,
        action.host,
      );
    ...
    case ADMIN_CONFIG_UPDATE_SUCCESS:
      return updateFromAdmin(
        state,
        fromJS(action.configs) as ImmutableList<ImmutableMap<string, any>>,
      );
    default:
      return state;
  }
