...
import { FE_NAME, SETTING_CHANGE, SettingsAction } from '../actions/settings';
...
const insertSuggestion = (
  ...
  compose.withMutations((map) => {
    map.updateIn(
      path,
      (oldText) =>
        `${(oldText as string).slice(0, position)}${completion} ${(oldText as string).slice(position + (token?.length ?? 0))}`,
    );
...
const insertEmoji = (
  ...
  const emojiText = isNativeEmoji(emojiData)
    ? emojiData.native
    : emojiData.colons;
...
const getExplicitMentions = (
  me: string,
  status: Pick<Status, 'content' | 'mentions'>,
) => {
  const fragment = domParser.parseFromString(
    status.content,
    'text/html',
...
const importAccount = (compose: Compose, account: CredentialAccount) => {
  ...
  return compose.withMutations((compose) => {
    if (settings.defaultPrivacy)
      compose.set('privacy', settings.defaultPrivacy);
    if (settings.defaultContentType)
      compose.set('content_type', settings.defaultContentType);
  ...
  switch (action.type) {
    ...
    case COMPOSE_SPOILER_TEXT_CHANGE:
      return updateCompose(state, action.composeId, (compose) => {
        return compose
          .setIn(
            compose.modified_language === compose.language
              ? ['spoiler_text']
              : ['spoilerTextMap', compose.modified_language],
            action.text,
          )
    ...
    case COMPOSE_VISIBILITY_CHANGE:
      return updateCompose(state, action.composeId, (compose) =>
        compose.set('privacy', action.value).set('idempotencyKey', uuid()),
      );
    ...
    case COMPOSE_CHANGE:
      return updateCompose(state, action.composeId, (compose) =>
        compose.set('text', action.text).set('idempotencyKey', uuid()),
      );
    case COMPOSE_REPLY:
      return updateCompose(state, action.composeId, (compose) =>
        compose.withMutations((map) => {
    ...
    case COMPOSE_SUBMIT_SUCCESS:
      return updateCompose(state, action.composeId, () =>
        state.get('default')!.withMutations((map) => {
          map.set('idempotencyKey', uuid());
          map.set(
            'in_reply_to',
            action.composeId.startsWith('reply:')
              ? action.composeId.slice(6)
              : null,
          );
    ...
    case COMPOSE_UPLOAD_SUCCESS:
      return updateCompose(state, action.composeId, (compose) =>
        appendMedia(compose, action.media, state.get('default')!.sensitive),
      );
    ...
    case COMPOSE_SET_STATUS:
      return updateCompose(state, 'compose-modal', (compose) =>
        compose.withMutations((map) => {
          ...
          map.set(
            'text',
            action.rawText || unescapeHTML(expandMentions(action.status)),
          );
          map.set('to', to);
          map.set('parent_reblogged_by', null);
          ...
          if (
            action.v?.software === PLEROMA &&
            action.withRedraft &&
            hasIntegerMediaIds(action.status)
          ) {
            map.set('media_attachments', ImmutableList());
          } else {
            map.set(
              'media_attachments',
              ImmutableList(action.status.media_attachments),
            );
          }
          ...
          if (action.poll) {
            map.set(
              ...
              PollRecord({
                options: ImmutableList(
                  action.poll.options.map(({ title }) => title),
                ),
    ...
    case COMPOSE_POLL_OPTION_ADD:
      return updateCompose(state, action.composeId, (compose) =>
        compose
          ...
          .updateIn(['poll', 'options_map'], (options_map) =>
            (options_map as ImmutableList<ImmutableMap<Language, string>>).push(
              ImmutableMap(compose.textMap.map((_) => action.title)),
            ),
          ),
      );
    case COMPOSE_POLL_OPTION_CHANGE:
      return updateCompose(state, action.composeId, (compose) =>
        compose.setIn(
          !compose.modified_language ||
            compose.modified_language === compose.language
            ? ['poll', 'options', action.index]
            : ['poll', 'options_map', action.index, compose.modified_language],
          action.title,
        ),
      );
    case COMPOSE_POLL_OPTION_REMOVE:
      return updateCompose(state, action.composeId, (compose) =>
        compose
          ...
          .updateIn(['poll', 'options_map'], (options_map) =>
            (
              options_map as ImmutableList<ImmutableMap<Language, string>>
            ).delete(action.index),
          ),
      );
        ...
        compose
          .setIn(
            !compose.modified_language ||
              compose.modified_language === compose.language
              ? ['editorState']
              : ['editorStateMap', compose.modified_language],
            action.editorState as string,
          )
          .setIn(
            !compose.modified_language ||
              compose.modified_language === compose.language
              ? ['text']
              : ['textMap', compose.modified_language],
    ...
    case COMPOSE_ADD_SUGGESTED_QUOTE:
      return updateCompose(state, action.composeId, (compose) =>
        compose.set('quote', action.quoteId),
      );
    case COMPOSE_ADD_SUGGESTED_LANGUAGE:
      return updateCompose(state, action.composeId, (compose) =>
        compose.set('suggested_language', action.language),
      );
    case COMPOSE_LANGUAGE_ADD:
      return updateCompose(state, action.composeId, (compose) =>
        compose
          ...
          .update('poll', (poll) => {
            if (!poll) return poll;
            return poll.update('options_map', (optionsMap) =>
              optionsMap.map((option, key) =>
                option.set(action.value, poll.options.get(key)!),
              ),
            );
          }),
      );
    ...
    case COMPOSE_QUOTE_CANCEL:
      return updateCompose(state, action.composeId, (compose) =>
        compose
          .update('dismissed_quotes', (quotes) =>
            compose.quote ? quotes.add(compose.quote) : quotes,
          )
          .set('quote', null),
      );
    case COMPOSE_FEDERATED_CHANGE:
      return updateCompose(state, action.composeId, (compose) =>
        compose.update('federated', (value) => !value),
      );
    default:
      return state;
  }
