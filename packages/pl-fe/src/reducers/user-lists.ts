...
const ReducerRecord = ImmutableRecord({
  ...
  event_participation_requests: ImmutableMap<
    string,
...
const userLists = (
  state = ReducerRecord(),
  action: DirectoryAction | AnyAction,
) => {
  switch (action.type) {
    case REBLOGS_FETCH_SUCCESS:
      return normalizeList(
        state,
        ['reblogged_by', action.statusId],
        action.accounts,
        action.next,
      );
    ...
    case DISLIKES_FETCH_SUCCESS:
      return normalizeList(
        state,
        ['disliked_by', action.statusId],
        action.accounts,
      );
    case REACTIONS_FETCH_SUCCESS:
      return state.setIn(
        ['reactions', action.statusId],
        ReactionListRecord({
          items: ImmutableOrderedSet<Reaction>(
            action.reactions.map(({ accounts, ...reaction }: APIEntity) =>
              ReactionRecord({
                ...reaction,
                accounts: ImmutableOrderedSet(
                  accounts.map((account: APIEntity) => account.id),
                ),
              }),
            ),
          ),
    ...
    case BIRTHDAY_REMINDERS_FETCH_SUCCESS:
      return normalizeList(
        state,
        ['birthday_reminders', action.accountId],
        action.accounts,
        action.next,
      );
    case FAMILIAR_FOLLOWERS_FETCH_SUCCESS:
      return normalizeList(
        state,
        ['familiar_followers', action.accountId],
        action.accounts,
        action.next,
      );
    case EVENT_PARTICIPATIONS_FETCH_SUCCESS:
      return normalizeList(
        state,
        ['event_participations', action.statusId],
        action.accounts,
        action.next,
      );
    ...
    case EVENT_PARTICIPATION_REQUESTS_FETCH_SUCCESS:
      return state.setIn(
        ['event_participation_requests', action.statusId],
        ParticipationRequestListRecord({
          next: action.next,
          items: ImmutableOrderedSet(
            action.participations.map(
              ({ account, participation_message }: APIEntity) =>
    ...
    case EVENT_PARTICIPATION_REQUESTS_EXPAND_SUCCESS:
      return state.updateIn(
        ...
        (items) =>
          (items as ImmutableOrderedSet<ParticipationRequest>).union(
            action.participations.map(
              ({ account, participation_message }: APIEntity) =>
                ParticipationRequestRecord({
                  account: account.id,
    ...
    case EVENT_PARTICIPATION_REQUEST_REJECT_SUCCESS:
      return state.updateIn(
        ['event_participation_requests', action.statusId, 'items'],
        (items) =>
          (items as ImmutableOrderedSet<ParticipationRequest>).filter(
            ({ account }) => account !== action.accountId,
          ),
      );
    case GROUP_BLOCKS_FETCH_SUCCESS:
      return normalizeList(
        state,
        ['group_blocks', action.groupId],
        action.accounts,
        action.next,
      );
