...
const listEditorReducer = (
  ...
  switch (action.type) {
      ...
      return state.update('accounts', (accounts) =>
        accounts.withMutations((map) => {
          ...
          map.set(
            'items',
            ImmutableList(
              action.accounts.map((item: { id: string }) => item.id),
            ),
          );
        }),
      );
    ...
    case LIST_EDITOR_SUGGESTIONS_READY:
      return state.setIn(
        ['suggestions', 'items'],
        ImmutableList(action.accounts.map((item: { id: string }) => item.id)),
      );
    ...
    case LIST_EDITOR_ADD_SUCCESS:
      return state.updateIn(['accounts', 'items'], (list) =>
        (list as ImmutableList<string>).unshift(action.accountId),
      );
    case LIST_EDITOR_REMOVE_SUCCESS:
      return state.updateIn(['accounts', 'items'], (list) =>
        (list as ImmutableList<string>).filterNot(
