...
import { type AnnouncementReaction, announcementReactionSchema } from 'pl-api';
...
const updateReaction = (
  ...
  announcementReactionSchema.parse({
    ...reaction,
...
const updateReactions = (
  ...
  if (idx > -1) {
    reactions = reactions.map((reaction) =>
      reaction.name === name
        ? updateReaction(reaction, count, me, overwrite)
        : reaction,
    );
  }
  ...
  return [
    ...reactions,
    updateReaction(
      announcementReactionSchema.parse({ name }),
      count,
      me,
      overwrite,
    ),
