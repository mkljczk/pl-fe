...
const useGroupMembershipRequests = (groupId: string) => {
  ...
  const path: ExpandedEntitiesPath = [
    Entities.ACCOUNTS,
    'membership_requests',
    groupId,
  ];
  ...
  const { entities, invalidate, fetchEntities, ...rest } = useEntities(
    ...
    () => client.experimental.groups.getGroupMembershipRequests(groupId),
    {
      ...
      enabled:
        relationship?.role === GroupRoles.OWNER ||
        relationship?.role === GroupRoles.ADMIN,
    },
  );

  const { dismissEntity: authorize } = useDismissEntity(
    path,
    async (accountId: string) => {
      const response =
        await client.experimental.groups.acceptGroupMembershipRequest(
          groupId,
          accountId,
        );
      invalidate();
  ...
  const { dismissEntity: reject } = useDismissEntity(
    path,
    async (accountId: string) => {
      const response =
        await client.experimental.groups.rejectGroupMembershipRequest(
          groupId,
          accountId,
        );
      invalidate();
