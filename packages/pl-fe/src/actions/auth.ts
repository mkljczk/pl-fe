...
import { fetchMeFail, fetchMeSuccess } from 'pl-fe/actions/me';
  ...
  dispatch(getAuthApp())
    ...
    .catch((error: { response: PlfeResponse }) => {
      if ((error.response?.json as any)?.error === 'mfa_required') {
        ...
        (error.response?.json as any)?.identifier === 'awaiting_approval'
      ) {
...
const register = (params: CreateAccountParams) => (dispatch: AppDispatch) => {
...
const authLoggedIn = (token: Token) => (dispatch: AppDispatch) => {
