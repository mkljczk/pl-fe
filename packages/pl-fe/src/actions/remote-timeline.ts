...
const getPinnedHosts = (state: RootState) => {
  ...
  return settings.getIn(['remote_timeline', 'pinnedHosts']) as
    | ImmutableList<string>
    | ImmutableOrderedSet<string>;
};

const pinHost =
  (host: string) => (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    return dispatch(
      changeSetting(
        ...
        pinnedHosts.toOrderedSet().add(host),
      ),
    );
  };
...
const unpinHost =
  (host: string) => (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    return dispatch(
      changeSetting(
        ['remote_timeline', 'pinnedHosts'],
        pinnedHosts.toOrderedSet().remove(host),
      ),
    );
  };
