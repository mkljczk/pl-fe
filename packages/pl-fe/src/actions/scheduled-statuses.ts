...
const SCHEDULED_STATUSES_FETCH_REQUEST =
  'SCHEDULED_STATUSES_FETCH_REQUEST' as const;
const SCHEDULED_STATUSES_FETCH_SUCCESS =
  'SCHEDULED_STATUSES_FETCH_SUCCESS' as const;
const SCHEDULED_STATUSES_FETCH_FAIL = 'SCHEDULED_STATUSES_FETCH_FAIL' as const;

const SCHEDULED_STATUSES_EXPAND_REQUEST =
  'SCHEDULED_STATUSES_EXPAND_REQUEST' as const;
const SCHEDULED_STATUSES_EXPAND_SUCCESS =
  'SCHEDULED_STATUSES_EXPAND_SUCCESS' as const;
const SCHEDULED_STATUSES_EXPAND_FAIL =
  'SCHEDULED_STATUSES_EXPAND_FAIL' as const;

const SCHEDULED_STATUS_CANCEL_REQUEST =
  'SCHEDULED_STATUS_CANCEL_REQUEST' as const;
const SCHEDULED_STATUS_CANCEL_SUCCESS =
  'SCHEDULED_STATUS_CANCEL_SUCCESS' as const;
...
const fetchScheduledStatuses =
  () => (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    return getClient(getState())
      .scheduledStatuses.getScheduledStatuses()
      .then(({ next, items }) => {
...
const cancelScheduledStatus =
  (statusId: string) => (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    return getClient(getState())
      .scheduledStatuses.cancelScheduledStatus(statusId)
      .then(() => {
...
const fetchScheduledStatusesSuccess = (
  statuses: Array<ScheduledStatus>,
  next: (() => Promise<PaginatedResponse<ScheduledStatus>>) | null,
) => ({
...
const expandScheduledStatuses =
  () => (dispatch: AppDispatch, getState: () => RootState) => {
    const next =
      (getState().status_lists.get('scheduled_statuses')
        ?.next as any as () => Promise<PaginatedResponse<ScheduledStatus>>) ||
      null;
