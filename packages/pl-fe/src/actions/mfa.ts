...
const MFA_BACKUP_CODES_FETCH_REQUEST =
  'MFA_BACKUP_CODES_FETCH_REQUEST' as const;
const MFA_BACKUP_CODES_FETCH_SUCCESS =
  'MFA_BACKUP_CODES_FETCH_SUCCESS' as const;
...
const fetchMfa = () => (dispatch: AppDispatch, getState: () => RootState) => {
  dispatch({ type: MFA_FETCH_REQUEST });
  return getClient(getState)
    .settings.mfa.getMfaSettings()
    .then((data) => {
...
const fetchBackupCodes =
  () => (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    return getClient(getState)
      .settings.mfa.getMfaBackupCodes()
      .then((data) => {
...
const setupMfa =
  (method: 'totp') => (dispatch: AppDispatch, getState: () => RootState) => {
    dispatch({ type: MFA_SETUP_REQUEST, method });
    return getClient(getState)
      .settings.mfa.getMfaSetup(method)
      .then((data) => {
