...
const FAVOURITED_STATUSES_FETCH_REQUEST =
  'FAVOURITED_STATUSES_FETCH_REQUEST' as const;
const FAVOURITED_STATUSES_FETCH_SUCCESS =
  'FAVOURITED_STATUSES_FETCH_SUCCESS' as const;
const FAVOURITED_STATUSES_FETCH_FAIL =
  'FAVOURITED_STATUSES_FETCH_FAIL' as const;

const FAVOURITED_STATUSES_EXPAND_REQUEST =
  'FAVOURITED_STATUSES_EXPAND_REQUEST' as const;
const FAVOURITED_STATUSES_EXPAND_SUCCESS =
  'FAVOURITED_STATUSES_EXPAND_SUCCESS' as const;
const FAVOURITED_STATUSES_EXPAND_FAIL =
  'FAVOURITED_STATUSES_EXPAND_FAIL' as const;

const ACCOUNT_FAVOURITED_STATUSES_FETCH_REQUEST =
  'ACCOUNT_FAVOURITED_STATUSES_FETCH_REQUEST' as const;
const ACCOUNT_FAVOURITED_STATUSES_FETCH_SUCCESS =
  'ACCOUNT_FAVOURITED_STATUSES_FETCH_SUCCESS' as const;
const ACCOUNT_FAVOURITED_STATUSES_FETCH_FAIL =
  'ACCOUNT_FAVOURITED_STATUSES_FETCH_FAIL' as const;

const ACCOUNT_FAVOURITED_STATUSES_EXPAND_REQUEST =
  'ACCOUNT_FAVOURITED_STATUSES_EXPAND_REQUEST' as const;
const ACCOUNT_FAVOURITED_STATUSES_EXPAND_SUCCESS =
  'ACCOUNT_FAVOURITED_STATUSES_EXPAND_SUCCESS' as const;
const ACCOUNT_FAVOURITED_STATUSES_EXPAND_FAIL =
  'ACCOUNT_FAVOURITED_STATUSES_EXPAND_FAIL' as const;
...
const fetchFavouritedStatusesSuccess = (
  ...
  () => (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    return next()
      .then((response) => {
        dispatch(importFetchedStatuses(response.items));
        dispatch(
          expandFavouritedStatusesSuccess(response.items, response.next),
        );
      })
      .catch((error) => {
...
const expandFavouritedStatusesSuccess = (
  statuses: Array<Status>,
  next: (() => Promise<PaginatedResponse<Status>>) | null,
) => ({
...
const fetchAccountFavouritedStatuses =
  (accountId: string) => (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    return getClient(getState)
      .accounts.getAccountFavourites(accountId)
      .then((response) => {
        dispatch(importFetchedStatuses(response.items));
        dispatch(
...
const fetchAccountFavouritedStatusesSuccess = (
  accountId: string,
  statuses: Array<Status>,
  next: (() => Promise<PaginatedResponse<Status>>) | null,
) => ({
...
const fetchAccountFavouritedStatusesFail = (
  accountId: string,
  error: unknown,
) => ({
...
const expandAccountFavouritedStatuses =
  (accountId: string) => (dispatch: AppDispatch, getState: () => RootState) => {
    if (!isLoggedIn(getState)) return;

    const next =
      getState().status_lists.get(`favourites:${accountId}`)?.next || null;

    if (
      next === null ||
      getState().status_lists.get(`favourites:${accountId}`)?.isLoading
    ) {
      return;
    }
...
const expandAccountFavouritedStatusesSuccess = (
  accountId: string,
  statuses: Array<Status>,
  next: (() => Promise<PaginatedResponse<Status>>) | null,
) => ({
...
const expandAccountFavouritedStatusesFail = (
  accountId: string,
  error: unknown,
) => ({
