...
const blockDomain =
  (domain: string) => (dispatch: AppDispatch, getState: () => RootState) => {
    if (!isLoggedIn(getState)) return;

    dispatch(blockDomainRequest(domain));

    return getClient(getState)
      .filtering.blockDomain(domain)
      .then(() => {
...
const unblockDomain =
  (domain: string) => (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    return getClient(getState)
      .filtering.unblockDomain(domain)
      .then(() => {
        const accounts = selectAccountsByDomain(getState(), domain);
        if (!accounts) return;
...
const fetchDomainBlocksSuccess = (
  domains: string[],
...
const selectAccountsByDomain = (state: RootState, domain: string): string[] => {
  const store = state.entities[Entities.ACCOUNTS]?.store as
    | EntityStore<Account>
    | undefined;
...
const expandDomainBlocksSuccess = (
  domains: string[],
  next: (() => Promise<PaginatedResponse<string>>) | null,
) => ({
