...
import { expandStatusQuotes, fetchStatusQuotes } from './status-quotes';
...
describe('fetchStatusQuotes()', () => {
  ...
  describe('with a successful API request', () => {
    beforeEach(async () => {
      ...
      __stub((mock) => {
        mock
          .onGet(`/api/v1/pleroma/statuses/${statusId}/quotes`)
          .reply(200, quotes, {
    ...
    beforeEach(() => {
      const state = rootState
        ...
        .set(
          'status_lists',
          ImmutableMap({
            [`quotes:${statusId}`]: StatusListRecord({ next: null }),
          }),
        );
      store = mockStore(state);
    });
    ...
    beforeEach(() => {
      const state = rootState
        ...
        .set(
          'status_lists',
          ImmutableMap({
            [`quotes:${statusId}`]: StatusListRecord({ next: 'example' }),
          }),
        );
      store = mockStore(state);
    });
