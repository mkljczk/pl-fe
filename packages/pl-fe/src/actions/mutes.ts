...
const initMuteModal = (account: Account) => (dispatch: AppDispatch) => {
  dispatch({
...
const toggleHideNotifications = () => (dispatch: AppDispatch) => {
  dispatch({ type: MUTES_TOGGLE_HIDE_NOTIFICATIONS });
};

const changeMuteDuration = (duration: number) => (dispatch: AppDispatch) => {
  dispatch({
