...
const messages = defineMessages({
  ...
  mutuals_only_short: {
    id: 'privacy.mutuals_only.short',
    defaultMessage: 'Mutuals-only',
  },
  mutuals_only_long: {
  ...
  local_long: {
    ...
    defaultMessage: 'Only visible on your instance',
  },
  list_short: { id: 'privacy.list.short', defaultMessage: 'List only' },
  list_long: {
    id: 'privacy.list.long',
    defaultMessage: 'Visible to members of a list',
  },
...
const PrivacyDropdown: React.FC<IPrivacyDropdown> = ({ composeId }) => {
  ...
  const items: Array<MenuItem> = options.map((item) => ({
    ...
    active:
      item.value === value || item.items?.some((item) => item.value === value),
