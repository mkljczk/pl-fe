...
} from 'pl-fe/features/preferences';
import {
  useAppDispatch,
  useAppSelector,
  useCompose,
...
const messages = defineMessages({
  languagePrompt: {
    id: 'compose.language_dropdown.prompt',
  ...
  multipleLanguages: {
    ...
    defaultMessage:
      '{count, plural, one {# more language} other {# more languages}}',
  },
  search: {
  ...
  addLanguage: {
    id: 'compose.language_dropdown.add_language',
...
const getLanguageDropdown =
  ...
  ({ handleClose: handleMenuClose }) => {
    ...
    const handleOptionClick: React.EventHandler<any> = (
    ...
    const handleAddLanguageClick: React.EventHandler<any> = (
      e: MouseEvent | KeyboardEvent,
    ) => {
      const value = (
        e.currentTarget as HTMLElement
      )?.parentElement?.getAttribute('data-index') as Language;
    ...
    const handleDeleteLanguageClick: React.EventHandler<any> = (
      e: MouseEvent | KeyboardEvent,
    ) => {
      const value = (
        e.currentTarget as HTMLElement
      ...
      <>
        <label className='relative block grow p-2 pt-1'>
          ...
          <div
            role='button'
            tabIndex={0}
            className='absolute inset-y-0 right-0 flex cursor-pointer items-center px-5 rtl:left-0 rtl:right-auto'
            onClick={handleClear}
          >
            <Icon
              ...
              src={
                isSearching
                  ? require('@tabler/icons/outline/backspace.svg')
        ...
        <div
          className='-mb-1 h-96 w-full overflow-auto'
          ref={node}
          tabIndex={-1}
        >
            ...
            return (
              <button
                ...
                className={clsx(
                  ...
                  {
                    'bg-gray-100 dark:bg-gray-800 black:bg-gray-900 cursor-pointer hover:bg-gray-200 dark:hover:bg-gray-700':
                      modified,
                    'cursor-pointer hover:bg-gray-100 black:hover:bg-gray-900 dark:hover:bg-gray-800':
                      !textMap.size || textMap.has(code),
                    'cursor-pointer': active,
                    'cursor-default':
                      !active && !(!textMap.size || textMap.has(code)),
                  },
                )}
              ...
              >
                <div
                  className={clsx(
                    'flex-auto grow text-primary-600 dark:text-primary-400',
                    {
                ...
                {features.multiLanguage &&
                  ...
                  (textMap.has(code) ? (
                    ...
                    >
                      <Icon
                        className='h-4 w-4'
                        src={require('@tabler/icons/outline/minus.svg')}
                      />
                    </button>
                  ) : (
                    <button
...
const LanguageDropdownButton: React.FC<ILanguageDropdownButton> = ({
  composeId,
}) => {
  const intl = useIntl();
  ...
  if (language) {
    ...
    buttonLabel = intl.formatMessage(messages.languageSuggestion, {
      language:
        languagesObject[suggestedLanguage as Language] || suggestedLanguage,
    });

  const LanguageDropdown = useMemo(
    () => getLanguageDropdown(composeId),
    [composeId],
  );

  return (
    <DropdownMenu component={LanguageDropdown}>
      <Button
        theme='muted'
        size='xs'
        text={buttonLabel}
