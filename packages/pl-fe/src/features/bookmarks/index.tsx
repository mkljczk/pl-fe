...
const messages = defineMessages({
  ...
  deleteFolderSuccess: {
    id: 'bookmarks.delete_folder.success',
...
const Bookmarks: React.FC<IBookmarks> = ({ params }) => {
  ...
  const statusIds = useAppSelector(
    (state) =>
      state.status_lists.get(bookmarksKey)?.items ||
      ImmutableOrderedSet<string>(),
  );
  const isLoading = useAppSelector(
    (state) => state.status_lists.get(bookmarksKey)?.isLoading === true,
  );
  const hasMore = useAppSelector(
    (state) => !!state.status_lists.get(bookmarksKey)?.next,
  );
  ...
  const handleDeleteFolder = () => {
    openModal('CONFIRM', {
      heading: intl.formatMessage(messages.deleteFolderHeading, {
        name: folder?.name,
      }),
