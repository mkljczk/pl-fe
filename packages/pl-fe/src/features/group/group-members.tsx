...
const GroupMembers: React.FC<IGroupMembers> = (props) => {
  ...
  const { groupMembers: owners, isFetching: isFetchingOwners } =
    useGroupMembers(groupId, GroupRoles.OWNER);
  const { groupMembers: admins, isFetching: isFetchingAdmins } =
  ...
  return (
    <>
      <ScrollableList
        ...
        showLoading={
          !group || isFetchingPending || (isLoading && members.length === 0)
        }
