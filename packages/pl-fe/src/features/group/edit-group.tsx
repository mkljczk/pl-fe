...
const messages = defineMessages({
  ...
  groupSaved: {
    id: 'group.update.success',
    defaultMessage: 'Group successfully saved',
  },
});
...
const EditGroup: React.FC<IEditGroup> = ({ params: { groupId } }) => {
  ...
  const avatar = useImageField({
    maxPixels: 400 * 400,
    preview: nonDefaultAvatar(group?.avatar),
  });
  const header = useImageField({
    maxPixels: 1920 * 1080,
    preview: nonDefaultHeader(group?.header),
  });
        ...
        >
          <Textarea
            ...
            placeholder={intl.formatMessage(
              messages.groupDescriptionPlaceholder,
            )}
            maxLength={maxNote}
            {...note}
          />
