...
import { resetListEditor, setupListEditor } from 'pl-fe/actions/lists';
...
const ListEditorModal: React.FC<BaseModalProps & ListEditorModalProps> = ({
  listId,
  onClose,
}) => {
  const intl = useIntl();
  ...
  const searchAccountIds = useAppSelector(
    (state) => state.listEditor.suggestions.items,
  );
