...
import { expandReblogs, fetchReblogs } from 'pl-fe/actions/interactions';
...
const ReblogsModal: React.FC<BaseModalProps & ReblogsModalProps> = ({
  onClose,
  statusId,
}) => {
  ...
  const accountIds = useAppSelector(
    (state) => state.user_lists.reblogged_by.get(statusId)?.items,
  );
  const next = useAppSelector(
    (state) => state.user_lists.reblogged_by.get(statusId)?.next,
  );
